...
namespace updater
{
	...
	{
		this->migrate_to_s1_naming(); //migrate to the s1-mod folder naming, will remove eventually 

		const auto data = this->base_ / "data";
		if (!utils::io::directory_exists(data.string()))
		{
			utils::io::create_directory(data.string());
		}
	...
	std::string file_updater::get_drive_filename(const file_info& file) const
	{
		...
		const auto path = this->base_ / file.name;
		return path.string();
	}

	void file_updater::move_current_process_file() const
	{
		utils::io::move_file(this->process_file_.string(), this->dead_process_file_.string());
	}

	void file_updater::restore_current_process_file() const
	{
		utils::io::move_file(this->dead_process_file_.string(), this->process_file_.string());
	}
	...
	void file_updater::cleanup_data_directory(const std::vector<file_info>& files) const
	{
		...
		const auto data_folder = base / "data";
		const auto existing_files = utils::io::list_files(data_folder.string());
	...
	void file_updater::migrate_to_s1_naming() const
	{
		//migrate to the s1 folder naming, will remove eventually 
		const auto s1x_folder = (this->base_ / "s1x").string();
		if (utils::io::directory_exists(s1x_folder))
		{
			const std::filesystem::path old_folder_path = s1x_folder;
			const std::filesystem::path new_folder_path = old_folder_path.parent_path() / "s1";
			...
			try
			{
				...
				{
					std::filesystem::remove_all(s1x_folder);
				}
			}
			catch (...)
			{
